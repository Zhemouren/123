...
void INTC_Handler (void)
{
	INT8U IntReg = INT_OUT;//中断输出寄存器
	
    if(IntReg  & TIMER_INT_OUT)//Timer中断触发
    { 
        if(TIM_GetITStatus(TIM_FLAG_Trigger))
        {
            TIM_ClearIT(TIM_FLAG_Trigger);
    ...
    if (IntReg & UART1_INT_OUT) //Uart1中断触发
	{
		uint8_t chr = UART1_RxData ;
		// uint8_t chr = UART1_FIFO ;
        // printf("uart1 recv:0x%x\n",chr);
		my_recv_dat_int(chr);

		INT_CLR = UART1_INT_CLR;//清除中断标志位
	}
	if (IntReg & UART0_INT_OUT) //Uart0中断触发
	{
		uint8_t chr = UART0_RxData ;
    	// uint8_t chr = UART0_FIFO ;
//        printf("uart0 recv:0x%x\n",chr);
    	my_recv_dat_int(chr);//接受数据
		INT_CLR = UART0_INT_CLR;//清除中断寄存器
	}
