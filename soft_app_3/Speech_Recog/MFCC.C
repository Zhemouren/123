...
u32* fft(s16* dat_buf, u16 buf_len)
{
	...
	my_fft(fft_out,fft_in,fft_point);
	...
	for(i=0;i<frq_max;i++)
	{
		...
		//soc_printf("%d %d",real,imag);

		real=real*real+imag*imag;
		//soc_printf(" %d",real);
		fft_out[i]=sqrtf_16((float)real)*10;
		//soc_printf(" %d\r\n",fft_out[i]);
	}
	return fft_out;
}
...
void get_mfcc(valid_tag *valid, v_ftr_tag *v_ftr, atap_tag *atap_arg)
{
	...
	else
	{
		...
		for(vc_dat=(u16*)(valid->start);vc_dat<=((u16*)(valid->end-frame_len));vc_dat+=(frame_len-frame_mov))
		{
			...
			for(i=0;i<frq_max;i++)
			{
				soc_printf("frq_spct[%d]=%d ",i,frq_spct[i]);
				frq_spct[i]*=frq_spct[i];//能量谱
				soc_printf("E_spct[%d]=%d\r\n",i,frq_spct[i]);
			}
			...
			for(h=0;h<tri_num;h++)
			{
				soc_printf("pow_spct[%d]=%d ",h,pow_spct[h]);
